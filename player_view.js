...
function createPlayerViewCharacterCard(character, team) {
    ...
    // 데이터가 아예 없을 경우를 대비해 안전하게 기본값([])을 할당합니다.
    const buffs = character.buffs || [];
    const debuffs = character.debuffs || [];
    const posX = character.posX !== undefined ? character.posX : -1;
    const posY = character.posY !== undefined ? character.posY : -1;

    let cardHTML = '';

    if (team === 'ally') {
        cardHTML = `
            <p><strong>${character.name || '알 수 없음'} (${character.type || '무'}) (${character.job || '무'})</strong> ${posX !== -1 ? `[${posX},${posY}]` : ''}</p>
            <p>HP: ${(character.currentHp || 0).toFixed(0)} / ${(character.maxHp || 0).toFixed(0)} ${character.shield > 0 ? `(+${character.shield.toFixed(0)}🛡️)` : ''}</p>
            <p>공격력: ${(character.atk || 0).toFixed(0)} | 마법 공격력: ${(character.matk || 0).toFixed(0)}</p>
            <p>방어력: ${(character.def || 0).toFixed(0)} | 마법 방어력: ${(character.mdef || 0).toFixed(0)}</p>
            <p>상태: ${character.isAlive ? '생존' : '<span style="color:red;">쓰러짐</span>'}</p>
            ${buffs.length > 0 ? `<p>버프: ${buffs.map(b => `${b.name || '버프'}(${b.turnsLeft || 0}턴${b.stacks > 1 ? `x${b.stacks}` : ''})`).join(', ')}</p>` : ''}
            ${debuffs.length > 0 ? `<p>디버프: ${debuffs.map(d => `${d.name || '디버프'}(${d.turnsLeft || 0}턴${d.stacks > 1 ? `x${d.stacks}`:''})`).join(', ')}</p>` : ''}
        `;
    } else { 
        cardHTML = `
            <p><strong>${character.name || '알 수 없음'} (${character.type || '무'})</strong> ${posX !== -1 ? `[${posX},${posY}]` : ''}</p>
            <p>상태: ${character.isAlive ? '생존' : '<span style="color:red;">쓰러짐</span>'}</p>
            ${buffs.length > 0 ? `<p>버프: ${buffs.map(b => `${b.name || '버프'}(${b.turnsLeft || 0}턴${b.stacks > 1 ? `x${b.stacks}` : ''})`).join(', ')}</p>` : ''}
            ${debuffs.length > 0 ? `<p>디버프: ${debuffs.map(d => `${d.name || '디버프'}(${d.turnsLeft || 0}턴${d.stacks > 1 ? `x${d.stacks}`:''})`).join(', ')}</p>` : ''}
        `;
